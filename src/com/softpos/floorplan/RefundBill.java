...
private void txtBillNoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtBillNoKeyPressed
    ...
    private Boolean loadDataFromBillNo() {
        ...
        if (billNoBean.getB_Void().equals("V")) {
            MSG.WAR(this, "ใบเสร็จรับเงินเลขที่ " + BillNo + " มีการทำรายการยกเลิก (Refund) ไปแล้ว...");
            initRefund();
            return RetValue;
        }
        if (billNoBean.getB_InvNo() != null && !billNoBean.getB_InvNo().equals("")) {
            MSG.WAR(this, "ใบเสร็จรับเงินนี้มีการพิมพ์ใบกำกับภาษีเต็มรูปเลขที่ " + billNoBean.getB_InvNo() + " กรุณายกเลิกใบกำกับภาษีเต็มรูปก่อนทำรายการ Refund ");
            initRefund();
            return RetValue;
        }
