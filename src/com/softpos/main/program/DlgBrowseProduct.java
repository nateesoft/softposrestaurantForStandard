...
public class DlgBrowseProduct extends javax.swing.JDialog {
    ...
    private GroupFileControl groupFileControl = AppContext.getGroupFileControl();
...
private void tblGroupKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tblGroupKeyReleased
    ...
    private void loadTblGroup() {
        ...
        List<GroupFileBean> list = groupFileControl.getAllData();

        for (GroupFileBean groupFile : list) {
        ...
        if (mdlGroup.getRowCount() != 0) {
            ...
            loadTableProductByGroup(tblGroup.getValueAt(0, 0).toString());
        }
    }

    private void loadTableProductByGroup(String productGroup) {
        JTableUtility.resetTableModel(mdlPlu);
        List<ProductBean> productList = productControl.getProductByPGroup(productGroup);
    ...
    private void onTblGroupSelectEvent() {
        ...
        if (index != -1) {
            loadTableProductByGroup(tblGroup.getValueAt(index, 0).toString());
        }
