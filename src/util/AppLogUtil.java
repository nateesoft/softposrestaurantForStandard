...
public class AppLogUtil {
    ...
    public static void shutdown() {
        ...
        if (allHandler != null) {
            try { allHandler.flush(); } catch (Exception ignore) {}
            try { allHandler.close(); } catch (Exception ignore) {}
            allHandler = null;
        }
        if (errorHandler != null) {
            try { errorHandler.close(); } catch (Exception ignore) {}
            errorHandler = null;
        }
    }

    /** Write raw HTML to logs/ folder for print debugging (kept for PrintDriver). */
